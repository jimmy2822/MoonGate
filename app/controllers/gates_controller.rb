class GatesController < ApplicationController
    before_action :find_gate_id, except: [:index, :show, :new, :create, :dislike, :like]
    before_action :authenticate_user! ,only: [:edit, :destory , :new, :create]

    # 將 Gate Model 資料傳進實體變數 @gates
    def index
        @gates = Gate.all
    end

    def show  
        @gate = Gate.find(params[:id])     
    end

    # 在 Controller 設定實體變數 @gate 給 view 的 form_for 使用
    def new
        @gate = scoped.new
    end
    
    # 將已過濾的資料存進實體變數 @gate 當中
    def create
        return redirect_to gates_path, notice: "新增門成功" if @gate.create(gate_params)
        # 儲存失敗時借用 new 方法的頁面，不使用 redirect_to 方法使用者在輸入時的資料不會消失
        render :new
    end

    # 取出 Gate 資料中 id 是 params[:id] 的資料
    def edit; end

    def update
        return redirect_to gate_path , notice: "更新成功" if @gate.update(gate_params) 
        render :edit, notice: "編輯失敗"
    end

    def destroy
        @gate.destroy
        redirect_to gates_path 
        # redirect_to gates_path, notice: "刪除成功"
    end

    def like
        Gate.find(params[:id]).like_logs.create(ip_address: request.remote_ip,who_liked: current_user[:id]) if @gate
        redirect_to gates_path, notice: "喜歡成功!"
    end

    def dislike
        @gate.destroy if @gate.like_logs.where(who_liked:current_user[:id])
    end

    private
    # 利用 Strong Parameters 設定過濾參數
    def gate_params
        params.require(:gate).permit(:name, :icon, :tag, :intro, :intro_detail, :is_public, :like, :server)
    end
    # 每個 Action 撈出 route 對應資料庫 id 的參數
    def find_gate_id
        @gate = scoped.find(params[:id])
    end

    def scoped
        return Gate if current_user.admin? && ['update', 'edit'].include?(params[:action])
        current_user.gates
    end
end
